...
public class AudioBook extends AudioContent
{
									...
									String author, String narrator, ArrayList<String> chapterTitles, ArrayList<String> chapters)
	{
		...
		//initializing variables for AudioBook class
		this.author = author;
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	public void play()
	{
		setAudioFile(getChapterTitles().get(currentChapter)+".\n"+getChapters().get(currentChapter));
		super.play();
	}
	...
	{
		int index = 0;
		for (String chapterTitle : this.chapterTitles) {
			index++;
			System.out.println("Chapter "+index+". "+chapterTitle);
		}
	}
	...
	public boolean equals(Object other)
	{
		...
		return super.equals(audioBook) && getAuthor().equals(audioBook.getAuthor()) && getNarrator().equals(audioBook.getNarrator());
	}
